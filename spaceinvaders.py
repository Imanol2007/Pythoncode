...
class Bullet:
    ...
    def render(self, screen):
        pygame.draw.rect(screen, (200, 200, 100), (self.x, self.y, self.width, self.height))
class Spaceship:
...
for i in range(aliensX):
    for j in range(aliensY):
        ...
        if j >= 3:
            alien_type = 1
        aliens.append(Alien(i*spaceX+10, j*spaceY+20, alien_type))
...
while not done:
    ...
    for i in range(player1.lives):
        pygame.draw.rect(screen, (200, 0, 0), (info.current_w-60-i*60, 20, 40, 40))
